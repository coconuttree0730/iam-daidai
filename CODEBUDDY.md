...
## 版式任务禁止用截图做验收（2026-09-10 用户指定）

**不要反复截图来看版式对不对。** 用户原话：「我手动运行就能看到问题，不需要你截图
截来截去的，浪费时间，你自己截图然后分析出来的效果又不准确」。

版式问题必须翻译成**可判定的数字**再动手，用无头浏览器读 DOM／几何，把数字报给用户，
视觉终判交给用户自己跑：
...
`qa/snap.mjs` 仍在，但**只在用户明确要图时才用**，且出图前必须确认不是空白页（~3KB 即失败）。

### 为什么：一次真实的返工

2026-09-10 用户连报三轮「卡片位置不对 / 红词挡脸」，前两轮我都在反复截图＋目测，
一轮没解决。第三轮换成先测后改，四条根因全部是量出来的（hover 把定位角归零导致
卡片沿半径飞 598.8px、外圈卡压标题、扇形轴心过低、英文标题横向溢出）——
**没有一条是靠眼睛看出来的**。
...
改动 `src/lib/pointer-frame.js`、`src/lib/demo-driver.js`、`src/lib/parallax.js`
或页面几何后必须重跑。
