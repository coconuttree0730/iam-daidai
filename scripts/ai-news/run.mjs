...
import { createEnricher } from './lib/translate.mjs';
...
// ⚠️ **必须转发 init**：调用方可能在 init 里指定 signal（每源的 fetchTimeoutMs）；
// 翻译模块（启用时）还用同一个 fetchImpl 发 POST，其 method / body / Authorization 全在 init 里。
// 早先本函数只接一个 url 参数、把 init 丢掉，接上翻译后就会静默退化成「无认证的 GET」——
// 那是接线期踩过的真实坑，故此处显式合并 init。
...
async function main() {
  ...
  // ── ticket 04：GLM 翻译（默认关闭）──────────────────────────────────
  // config.translate=false 时**完全不构造 enricher**、也不读 GLM_API_KEY：
  // 这样默认路径零第三方数据流、零密钥依赖，少一个失败分支（站长优先项）。
  // 开启条件 = 配置开关 AND 环境里有密钥；缺密钥时降级为「不翻译」而不是报错。
  const apiKey = CONFIG.translate ? process.env.GLM_API_KEY : undefined;
  const enrich = apiKey ? createEnricher({ fetchImpl: realFetch, apiKey }) : undefined;
  if (CONFIG.translate && !apiKey) {
    console.log('提示：已开启翻译但未设置 GLM_API_KEY，本轮跳过翻译（条目以原文落盘、标记未翻译）。');
  }
  ...
  const translated = (result.window ?? []).filter((it) => it.translated === true).length;
  const archived = (result.archives ?? []).reduce((n, a) => n + (a.items?.length ?? 0), 0);
  console.log(
    `written: ${result.kept} 条（其中已翻译 ${translated} 条）, 归档 +${archived} 条, updated=${result.updated}`,
  );
  process.exitCode = 0;
}
