...
W="${1:-1440}"; H="${2:-900}"
export TMPDIR="$ROOT/.tmp"; mkdir -p "$TMPDIR/crash" "$TMPDIR/cdp-ev"
export ASTRO_TELEMETRY_DISABLED=1
HTTP_PORT=8903; CDP_PORT=9352
cd "$ROOT/dist" || exit 1
python3 -m http.server $HTTP_PORT --bind 127.0.0.1 > "$TMPDIR/http-ev.log" 2>&1 &
HTTP_PID=$!; sleep 2
...
google-chrome-stable --headless=new --no-sandbox --disable-gpu --disable-dev-shm-usage --no-proxy-server \
  --breakpad-dump-location="$TMPDIR/crash" --user-data-dir="$TMPDIR/cdp-ev" \
  --remote-debugging-port=$CDP_PORT about:blank > "$TMPDIR/cdp-ev.log" 2>&1 &
CHROME_PID=$!
...
cd "$ROOT"; CDP_PORT=$CDP_PORT node qa/diag-elem-vectors.mjs "http://127.0.0.1:$HTTP_PORT/index.html" qa/out "$W" "$H"
